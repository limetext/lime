...
func (ve *ViewEvent) Call(v *View) {
	log.LogFinest("%s(%v)", evNames[ve], v.Id())
	for _, ev := range *ve {
		ev(v)
	}
}
...
func (qe QueryContextEvent) Call(v *View, key string, operator util.Op, operand interface{}, match_all bool) QueryContextReturn {
	log.LogFine("Query context: %s, %v, %v, %v", key, operator, operand, match_all)
	for i := range qe {
	...
	log.LogFine("Unknown context: %s", key)
	return Unknown
}
...
func (we *WindowEvent) Call(w *Window) {
	log.LogFinest("%s(%v)", wevNames[we], w.Id())
	for _, ev := range *we {
		ev(w)
	}
}
