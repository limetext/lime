...
func (v *View) parsethread() {
	...
	doparse := func() (ret bool) {
		...
		defer func() {
			if r := recover(); r != nil {
				log.LogError("Panic in parse thread: %v\n%s", r, string(debug.Stack()))
				if pc > 0 {
					panic(r)
				}
				pc++
		...
		if err != nil {
			log.LogError("Couldn't parse: %v", err)
			return
		}

		syn, err := parser.NewSyntaxHighlighter(pr)
		if err != nil {
			log.LogError("Couldn't create syntaxhighlighter: %v", err)
			return
		}
...
func (v *View) EndEdit(edit *Edit) {
	if edit.invalid {
		...
		log.LogFine("This edit has already been invalidated: %v, %v", edit, v.editstack)
		return
	}
	...
	if i == -1 {
		...
		log.LogError("This edit isn't even in the stack... where did it come from? %v, %v", edit, v.editstack)
		return
	}
	...
	if l := len(v.editstack) - 1; i != l {
		...
		log.LogError("This edit wasn't last in the stack... %d !=  %d: %v, %v", i, l, edit, v.editstack)
	}
...
func (v *View) runCommand(cmd TextCommand, name string) error {
	...
	defer func() {
		v.EndEdit(e)
		if r := recover(); r != nil {
			log.LogError("Paniced while running text command %s %v: %v\n%s", name, cmd, r, string(debug.Stack()))
		}
	}()
